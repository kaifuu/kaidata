...
### 现代湖仓架构的企业级数据中台 · 9 大域 40+ 子模块

多源接入 → Kafka 总线 → StarRocks 实时数仓 → 治理 / 开发 / 服务 → 前端门户，全链路真实流转
...
单机 Docker Compose 一键部署：**MinIO + Kafka + Flink + StarRocks**。
...
- **大数据**：StarRocks 3.3（MySQL 协议数仓）/ Kafka 3.7（KRaft）/ Flink 1.18 / MinIO
...
```
kaidata/
├── datalake-service/     # 服务层：Spring Boot REST（9 大域后端）
