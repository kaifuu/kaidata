...
export const api = {
  ...
  govElementVersions: (elementId: number) => http.get('/data-gov/std/element/versions', { params: { elementId } }).then((r) => r.data),
  govElementVersionCompare: (elementId: number, v1: number, v2: number) => http.get('/data-gov/std/element/version-compare', { params: { elementId, v1, v2 } }).then((r) => r.data),
  govElementExcel: () => http.get('/data-gov/std/element/excel', { responseType: 'blob' }).then((r) => r.data),
  govElementImport: (file: File) => { const fd = new FormData(); fd.append('file', file); return http.post('/data-gov/std/element/import', fd, { timeout: 60000 }).then((r) => r.data); },
  govElementRecommend: (metaId: number) => http.get('/data-gov/std/element/recommend', { params: { metaId } }).then((r) => r.data),
  // 治理驾驶舱 + 标准落标/合规
  ...
  govStdComplianceScan: (live = false) => http.get('/data-gov/std/compliance-scan', { params: { live } }).then((r) => r.data),
  ...
  govModelRelations: (modelId: number) => http.get('/data-gov/model/relation', { params: { modelId } }).then((r) => r.data),
  govSaveModelRelation: (b: any) => http.post('/data-gov/model/relation', b).then((r) => r.data),
  govDeleteModelRelation: (id: number) => http.delete('/data-gov/model/relation', { params: { id } }).then((r) => r.data),
  govModelEr: (modelId: number) => http.get('/data-gov/model/er', { params: { modelId } }).then((r) => r.data),
  govModelVersionList: (modelId: number) => http.get('/data-gov/model/version/list', { params: { modelId } }).then((r) => r.data),
  govModelVersionCreate: (modelId: number, detail: string) => http.post('/data-gov/model/version', null, { params: { modelId, detail } }).then((r) => r.data),
  govModelVersionCompare: (modelId: number, v1: number, v2: number) => http.get('/data-gov/model/version/compare', { params: { modelId, v1, v2 } }).then((r) => r.data),
  ...
  govQualityIssues: (params: { status?: string; taskId?: number; severity?: string; keyword?: string; overdue?: number } = {}) => http.get('/data-gov/quality/issue/list', { params }).then((r) => r.data),
  govQualityIssueStats: () => http.get('/data-gov/quality/issue/stats').then((r) => r.data),
  govQualityIssueUsers: () => http.get('/data-gov/quality/issue/users').then((r) => r.data),
  govQualityIssueLog: (issueId: number) => http.get('/data-gov/quality/issue/log', { params: { issueId } }).then((r) => r.data),
  govQualityIssueAssign: (id: number, assignee: string, deadline?: string, comment?: string) => http.post('/data-gov/quality/issue/assign', null, { params: { id, assignee, deadline, comment } }).then((r) => r.data),
  govQualityIssueBatchAssign: (ids: string, assignee: string, deadline?: string, comment?: string) => http.post('/data-gov/quality/issue/batch-assign', null, { params: { ids, assignee, deadline, comment } }).then((r) => r.data),
  govQualityIssueProcess: (id: number, comment?: string) => http.post('/data-gov/quality/issue/process', null, { params: { id, comment } }).then((r) => r.data),
  govQualityIssueResolve: (id: number, comment: string) => http.post('/data-gov/quality/issue/resolve', null, { params: { id, comment } }).then((r) => r.data),
  govQualityIssueClose: (id: number, comment?: string) => http.post('/data-gov/quality/issue/close', null, { params: { id, comment } }).then((r) => r.data),
  govQualityIssueReject: (id: number, comment?: string) => http.post('/data-gov/quality/issue/reject', null, { params: { id, comment } }).then((r) => r.data),
  govQualityIssueReopen: (id: number, comment?: string) => http.post('/data-gov/quality/issue/reopen', null, { params: { id, comment } }).then((r) => r.data),
  govQualityIssueSampleCsv: (id: number) => http.get('/data-gov/quality/issue/sample-csv', { params: { id }, responseType: 'blob' }).then((r) => r.data),
  govQualityDryRun: (b: any) => http.post('/data-gov/quality/rule/dry-run', b, { timeout: 60000 }).then((r) => r.data),
  ...
  govTagRules: () => http.get('/data-gov/tag/rule').then((r) => r.data),
  govSaveTagRule: (b: any) => http.post('/data-gov/tag/rule', b).then((r) => r.data),
  govDeleteTagRule: (id: number) => http.delete('/data-gov/tag/rule', { params: { id } }).then((r) => r.data),
  govTagRuleApply: () => http.post('/data-gov/tag/rule/apply', null, { timeout: 120000 }).then((r) => r.data),
  govTagInherit: () => http.post('/data-gov/tag/inherit', null, { timeout: 60000 }).then((r) => r.data),
  // 主数据
  ...
  govSaveMasterRecord: (b: any) => save('/data-gov/master/record', b),
  govDeleteMasterRecord: (id: number) => http.delete('/data-gov/master/record', { params: { id } }).then((r) => r.data),
  govMasterAudit: (masterId: number) => http.get('/data-gov/master/record/audit', { params: { masterId } }).then((r) => r.data),
  govMasterRefs: (id: number) => http.get('/data-gov/master/refs', { params: { id } }).then((r) => r.data),
  govMasterCheckDuplicate: (masterId: number, recordId: number, code: string) => http.get('/data-gov/master/record/check-duplicate', { params: { masterId, recordId, code } }).then((r) => r.data),
