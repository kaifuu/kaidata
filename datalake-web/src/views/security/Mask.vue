  ...
  <div class="dl-card">
    ...
    <el-tabs v-model="tab">
      <el-tab-pane label="脱敏规则" name="rule">
        ...
        <el-table :data="pagedRules" size="small" stripe border v-loading="loading">
      ...
      <el-tab-pane label="字段绑定" name="rel">
        ...
        <el-table :data="pagedRels" size="small" stripe border>
        ...
        <div class="dl-pagination">
          <el-pagination :current-page="relsPage.page" :page-size="relsPage.size" :total="rels.length"
            :page-sizes="[10, 20, 50]" layout="total, sizes, prev, pager, next, jumper" size="small" background
            @size-change="onRelsSizeChange" @current-change="onRelsPageChange" />
        </div>
      </el-tab-pane>
    </el-tabs>

    <el-drawer v-model="ruleDlg" :title="ruleForm.id ? '编辑规则' : '新增规则'" size="620px">
    ...
    <el-drawer v-model="relDlg" title="绑定字段" size="560px">
...
const relsPage = reactive({ page: 1, size: 10 })
const pagedRels = computed(() => rels.value.slice((relsPage.page - 1) * relsPage.size, relsPage.page * relsPage.size))
function onRelsSizeChange(s: number) { relsPage.size = s; relsPage.page = 1 }
function onRelsPageChange(p: number) { relsPage.page = p }
